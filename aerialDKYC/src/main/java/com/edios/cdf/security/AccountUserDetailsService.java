...
public class AccountUserDetailsService implements UserDetailsService {
	...
	public UserDetails fetchUserByUsername(String username, UsernamePasswordAuthenticationToken authenticationToken)
		...
		System.out.println(username+"In fetchUserByUsername() function"+password);
		...
		if (userDetailTO != null) {
			...
			} else if (password.equals(userDetailTO.getEmailPassword())) {
				throw new BadCredentialsException(messageSource.getMessage("updatePasswordAfterForgot", null,
						"ResetForgotPassword", Locale.US)+userDetailTO.getUserID());
			} else {
...
public static void main(String[] args) {
	WebAppied appied = new WebAppied();
	System.out.println(appied.decrypt("We5f5iIFC+Y="));
}
}
